...
const OrderEdit = (props) => {
  ...
  const [internalproductList, setinternalProductList] = useState([]);
  ...
  const [searchText,setSearchText] = useState('');
   ...
   const { data, error } = useSWR(useSwrFlag?server_ip+`getAllInternalPaginatedItemsForBundle?page=${pageIndex}&search=${search}`:null, fetcher);
 ...
 async function updateOrder(orderNo,deliveryCharges,totalBill,deletedProduct,updatedProduct,OrderStatus,OrderShippingAddress){
  const data = await api.updateOrder(orderNo,deliveryCharges,totalBill,deletedProduct,updatedProduct,OrderStatus,OrderShippingAddress,session.accessToken).then((response) => {
    if(response.data['ErrorCode']==1){
      ...
      if(OrderStatus=='CONFIRMED'){saveTODb(orderNo);}
      else{
...
// SENT TO POS //
async function saveTODb(orderNo){
  // const payload = {
    
  // };
  try {
    const response1 = await fetch(`${server_ip}saveOrderDB`,{method:'POST',body:JSON.stringify({'orderNo':orderNo}),headers: {"Authorization":"Bearer "+session.accessToken,'Content-Type': 'application/json'}})
    const payload = await response1.json();
    if(payload['ErrorCode']==0){
      toast.success('Order Confirmed Sucessfully! '+payload['ErrorMsg'], {position: toast.POSITION.TOP_RIGHT});
      // window.location.href = '/admin/orders';
      router.push({
        pathname: `/admin/orders`,
        query: { pageIndexRouter: router.query.pageIndexRouter, scrollPosition:router.query.scrollPosition, rowsPerPageRouter:router.query.rowsPerPageRouter },
    ...
    if (error.response) {
        toast.error('Error Occured While Order Saving! Kindly Contact Administrator '+error.response.statusText, {position: toast.POSITION.TOP_RIGHT});
        return error.response
