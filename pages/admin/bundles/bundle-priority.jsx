...
import { BundlePriorityForm } from "pages-sections/admin";
...
BundlePriority.getLayout = function getLayout(page) {
  return <VendorDashboardLayout>{page}</VendorDashboardLayout>;
}; // =============================================================================

// =============================================================================
export default function BundlePriority(props) {
  ...
  const { data, error } = useSWR(useSwrFlag?`${server_ip}getBundleTypes`:null, fetcher);
  ...
  async function getBundleForPrioritySet(bundleType){
    const myNewModel = await axiosInstance
      .post(`${server_ip}getBundleForPrioritySet`, {bundleType:bundleType}, {
          headers: {
  ...
  const handleBundleSelect = (type) => {
    setPriorityType(type);
    getBundleForPrioritySet(type);
  }
  ...
  if(state['tasks'].length > 1){
    ...
    const myNewModel = await axiosInstance
      .post(`${server_ip}updateBundlePriority`, {tasks:state['tasks'],priorityType:priorityType}, {
          headers: {
              "Authorization":'Bearer '+session.accessToken,
          },
      }).then((res) => {
    ...
    else if(state['tasks'].length == 1) {toast.info('Bundle should have more than one record to set priority.', {position: toast.POSITION.TOP_RIGHT});}
    else{toast.info('Kindly select any bundle type to update priority.', {position: toast.POSITION.TOP_RIGHT});}
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <BundlePriorityForm bundles={BundleList} handleBundleSelect={handleBundleSelect} />
        </Flex>
