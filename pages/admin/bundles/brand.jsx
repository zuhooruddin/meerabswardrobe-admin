...
SchoolBundleList.getLayout = function getLayout(page) {
  return <VendorDashboardLayout>{page}</VendorDashboardLayout>;
}; // =============================================================================

// =============================================================================
export default function SchoolBundleList(props) {
  const router = useRouter();
  const { mutate } = useSWRConfig()
  const { data: session,status} = useSession();
  const [useSwrFlag,setUseSwrFlag] = useState(true);
  const [useScrollFlag,setUseScrollFlag] = useState(false);
  const [schoolBundleList, setSchoolBundleList] = useState(null);
  ...
  const { data, error } = useSWR(useSwrFlag?server_ip+`getAllBrandBundle?page=${pageIndex+1}${'&page_size='+rowsPerPage}${search?'&search='+search:''}`:null, fetcher);
  if (error) <p>Loading failed...</p>;
  if (!data) <h1>Loading...</h1>;
  if(useSwrFlag && data && data!=schoolBundleList){
    setSchoolBundleList(data);setUseSwrFlag(false);
    ...
    const myNewModel = await axiosInstance
      .post(`${server_ip}deleteBundle`, id, {
          ...
          if(res.data['ErrorCode']==0 ){
            setUseSwrFlag(true);
            mutate(server_ip+`getAllBrandBundle?page=${pageIndex}&search=${search}`);
  ...
  const {
    ...
    listData: schoolBundleList?schoolBundleList['results']:[],
    rowsPerPage: rowsPerPage,
  });

  return (
    <Box py={4}>
      <H3 mb={2}>Brand Bundles</H3>
      ...
      <SearchArea
        ...
        searchPlaceholder="Search Brand Bundle..."
          ...
          >
            <Table>
              <TableHeader
                ...
                rowCount={schoolBundleList?schoolBundleList['results'].length:0}
      ...
      <TableRow>
        ...
        <Pagination count={schoolBundleList?Math.ceil(schoolBundleList['count']/20):''} variant="outlined" color="primary" onChange={handleChange} />
        <Span color="grey.600" style={{marginTop:'1%'}}>Showing {schoolBundleList?((pageIndex-1)*20)+1:''}-{schoolBundleList?((pageIndex-1)*20) + schoolBundleList['results'].length:''} of {schoolBundleList?schoolBundleList['count']:''} Schools</Span>
        </Stack> */}
        <TablePagination
          component="div"
          count={schoolBundleList?schoolBundleList['count']:''}
          page={pageIndex}
