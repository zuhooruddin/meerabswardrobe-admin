...
EditCourier.getLayout = function getLayout(page) {
  return <VendorDashboardLayout>{page}</VendorDashboardLayout>;
}; // =============================================================================

export default function EditCourier(props) {
  const router = useRouter();
//   const { data: session, status } = useSession()
  const { data: session, status } = useSession()

    if(props.data[0]['name']==null){props.data[0]['name']='';}
    if(props.data[0]['shortCode']==null){props.data[0]['shortCode']='';}
    ...
    const initialValues = {
      ...
      shortCode: props.data[0]['shortCode'],
      status: props.data[0]['status'],
    };
    
    async function updateCourier(data,id,setdisableButtonCheck){

      const configurationsModel = await axiosInstance
        .patch(server_ip+`updateCourier/${id}`, data, {
          headers: {
          ...
          if(res.status == 200){
            toast.success("Courier Updated Successfully", {position: toast.POSITION.TOP_RIGHT});
            // router.push('/admin/courier', null, { shallow: true });
            ...
            if(error.response.status==400){
              ...
              toast.error('Error Occured while updating Courier: '+error.response.statusText, {position: toast.POSITION.TOP_RIGHT});
            }
            return error.response
    ...
    const handleFormSubmit = (values) => {
      ...
      if(values.shortCode!=initialValues['shortCode']){formData.append("shortCode", values['shortCode']);}
      if(values.status!=initialValues['status']){formData.append("status", values['status']);}
      updateCourier(formData,values.id,setdisableButtonCheck);
    };
